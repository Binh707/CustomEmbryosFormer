...
class InferEmbryoFormer(nn.Module):
    """ This is the PDVC module that performs dense video captioning """
    ...
    def forward(self, dt):
        ...
        out = self.predict(dt, hs, init_reference, inter_references, others)
        return out
    ...
    def predict(self, dt, hs, init_reference, inter_references, others):
        outputs_classes = []
        outputs_coords = []
        outputs_widths = []
        outputs_centers = []
        ...
        out['refine_weight_attn'] = others['refine_weight_attn']
        out['pred_frames_refine'] = self.get_frame_refine_prediction(out, dt['video_target'], pos_weight=dt['video_pos_weight'])

        return out


    def get_frame_refine_prediction(self, outputs, targets, pos_weight):
